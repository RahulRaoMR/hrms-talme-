...
function shouldUseResend() {
  ...
  if (provider === "resend") return hasResendKey();
  if (provider === "smtp" || provider === "gmail") return false;
  if (service === "resend") return hasResendKey();
...
export function isEmailConfigured() {
  ...
  if (process.env.SMTP_HOST) return hasSmtpCredentials();
...
function sendWithSmtp(to, subject, html, options = {}) {
  return Promise.race([
