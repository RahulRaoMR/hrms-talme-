...
const dailyUpdatesSeedPath = path.join(process.cwd(), "data", "daily-updates.json");

function getDailyUpdatesSeed() {
  try {
    if (!fs.existsSync(dailyUpdatesSeedPath)) return [];
    const parsed = JSON.parse(fs.readFileSync(dailyUpdatesSeedPath, "utf8"));
...
const seedLoaders = {
  ...
  "punch-activity": () => [],
  "shift-assignments": () => [],
  "daily-updates": getDailyUpdatesSeed
};
...
export function getLocalSuiteData() {
  return {
    ...
    punchActivity: getResource("punch-activity") || [],
...
export function listActivityLogs(limit = 500) {
  return [...(getResource("activity") || [])]
...
export function createActivityLog(payload = {}) {
  const store = getStore();
  const row = {
    id: payload.id || `activity-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  ...
  store.activity = [row, ...(store.activity || [])].slice(0, 1000);
  writeStoreFile(store);
  return row;
}
...
export function deleteActivityLog(id) {
  const store = getStore();
  const beforeCount = (store.activity || []).length;

  store.activity = (store.activity || []).filter((entry) => String(entry.id) !== String(id));
  writeStoreFile(store);
  return store.activity.length !== beforeCount;
}
