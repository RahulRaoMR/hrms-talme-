  ...
  recruiters: {
    ...
    async createData(payload) {
      return {
        ...
        name: requiredString(payload, "name", "Name"),
        email: optionalString(payload, "email")?.toLowerCase(),
  ...
  attendanceRecords: {
    ...
    async createData(payload) {
      return {
        employee: requiredString(payload, "employee", "Employee"),
        salaryNetPay: optionalNumber(payload, "salaryNetPay", "Salary net pay") || 0,
        month: optionalString(payload, "month") || "",
        monthDays: optionalNumber(payload, "monthDays", "Month days") || 0,
        sundays: optionalNumber(payload, "sundays", "Sundays") || 0,
        holidays: optionalNumber(payload, "holidays", "Holidays") || 0,
        paidLeaves: optionalNumber(payload, "paidLeaves", "Paid leaves") || 0,
        otHours: optionalNumber(payload, "otHours", "OT hours") || 0,
        present: requiredNumber(payload, "present", "Present days"),
        leaves: optionalNumber(payload, "leaves", "Leaves") ?? optionalNumber(payload, "paidLeaves", "Paid leaves") ?? 0,
        overtime: optionalNumber(payload, "overtime", "Overtime") ?? optionalNumber(payload, "otHours", "OT hours") ?? 0,
        shift: optionalString(payload, "shift") || "Salary Net Pay",
        lockState: optionalString(payload, "lockState") || "Salary Ready",
        tone: optionalString(payload, "tone") || "gold"
      };
    },
    async updateData(payload) {
      return {
        ...
        salaryNetPay: optionalNumber(payload, "salaryNetPay", "Salary net pay"),
        month: optionalString(payload, "month"),
        monthDays: optionalNumber(payload, "monthDays", "Month days"),
        sundays: optionalNumber(payload, "sundays", "Sundays"),
        holidays: optionalNumber(payload, "holidays", "Holidays"),
        paidLeaves: optionalNumber(payload, "paidLeaves", "Paid leaves"),
        otHours: optionalNumber(payload, "otHours", "OT hours"),
