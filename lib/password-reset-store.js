...
export async function getLocalLoginAccount(identifier, password, expectedRole) {
  ...
  if (["Employee", "Employee HRMS"].includes(resolvedRole)) {
    ...
    const savedPassword = resetStore.passwords.get(employeeEmail) || resetStore.passwords.get(employeeKey);
    const savedRole = normalizeRole(savedPassword?.role) || resolvedRole;

    if (!employee || !employeeKey || (resolvedRole && savedRole !== resolvedRole) || !passwordMatches(savedPassword, password)) {
      return null;
    }
    ...
    return {
      ...
      email: employeeEmail || employeeKey,
      role: savedRole,
      employeeId: employee.employeeId || null
    };
  }

  if (!account || (resolvedRole && normalizeRole(account.role) !== resolvedRole)) {
