...
export async function getLocalLoginAccount(identifier, password, expectedRole) {
  ...
  if (resolvedRole === "Employee") {
    ...
    const savedPassword = resetStore.passwords.get(employeeKey);

    if (!employee || !employeeKey || !passwordMatches(savedPassword, password)) {
      return null;
    }
    ...
    return {
      ...
      email: normalizeEmail(employee.email) || employeeKey,
      role: "Employee",
